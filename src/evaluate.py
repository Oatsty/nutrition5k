...
def train(config: CN, model: nn.Module, loss_func: Callable, criterion: nn.Module, device: torch.device):
  print(' '.join(config.TITLE))
  batch_size = config.TRAIN.BATCH_SIZE
  resize_size = (config.EVAL.HEIGHT, config.EVAL.WIDTH)
  # データセットの準備
  dataset = make_dataset(config)
  dataloader = DataLoader(dataset['test'], batch_size=batch_size, num_workers=8, shuffle=False, collate_fn=collate_fn)
  running_loss = 0.0
  running_loss_multi = {}
  with torch.no_grad():
