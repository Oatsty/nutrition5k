...
class CookpadDataset(BaseDataset):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.metadatas_dict = json.load(open('metadata/cookpad/metadatas_refined_path.json'))
        self.mean_metadata = Metadata('mean', 0.,0.,0.,0.,0.)
        self.std_metadata = Metadata('std', 1.,1.,1.,1.,1.)
    ...
    def get_metadata(self, dish_id: str):
        ...
        data_dict.cal = (df['ENERC_KCAL'].dropna().sum() - mean_metadata.cal) / std_metadata.cal
        data_dict.mass = (df['Weight'].dropna().sum() - mean_metadata.mass) / std_metadata.mass
        data_dict.fat = (df['FAT-'].dropna().sum() - mean_metadata.fat) / std_metadata.fat
        data_dict.carb = (df['CHOAVLDF-'].dropna().sum() - mean_metadata.carb) / std_metadata.carb
        data_dict.protein = (df['PROT-'].dropna().sum() - mean_metadata.protein) / std_metadata.protein
        data_dict.ingrs = []
    ...
    def __getitem__(self, index: int) -> dict[str,Any]:
        ...
        sample = {'rgb_img': rgb_img, 'depth_img': depth_img, 'metadata': metadata, 'rgb_path': str(img_path), 'depth_path': str(img_path)}
        return sample
...
def make_dataset(config: Optional[CN], imgs_dir: str = '.', metadatas_path: str = '.', splits_train_path: str = '.', splits_test_path: str = '.', unnormalized_int_tensor: bool = False) -> dict[str, BaseDataset]:
    ...
    train_dataset = CookpadDataset(imgs_dir_p, metadatas_path_p, splits_train, transform=transform, normalize_depth=normalize_depth)
    test_dataset = CookpadDataset(imgs_dir_p, metadatas_path_p, splits_test, transform=transform, normalize_depth=normalize_depth)
