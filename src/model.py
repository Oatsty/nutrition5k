...
class BaseModel(nn.Module,ABC):
    ...
    def __call__(self, rgb_img: torch.Tensor, depth_img: torch.Tensor, **kwargs) -> dict[str,torch.Tensor]:
...
class FPN(BaseModel):
    def __init__(self, hidden_dim: int, pretrained_model: str = 'microsoft/resnet-50', resolution_level: int = 2, regressor: BaseRegressor = Regressor(2048,2048)) -> None:
...
class FPNOpenSeeD(BaseModel):
    def __init__(self, hidden_dim: int, regressor: BaseRegressor, resolution_level: str = 'res4', mask_weight: float = 0.5, device='cuda') -> None:
...
class FPNSwin(BaseModel):
    ...
    def forward(self, rgb_img: torch.Tensor, depth_img: torch.Tensor, mask: torch.Tensor, skip_attn: bool = False, **kwargs):
        ...
        depth_features = self.depth_backbone.forward(depth_img,output_hidden_states=True)
        ...
        for i, hid in enumerate(rgb_features_hid[:-1]):
            ...
            m = f.adaptive_avg_pool2d(mask.float(),(hid.shape[-2],hid.shape[-1])).unsqueeze(1)
