...
from .utils.dino_decoder import DeformableTransformerDecoderLayer, TransformerDecoder
...
class OpenSeeDDecoder(nn.Module):
    ...
    def forward(
        ...
        if self.two_stage:
            ...
            enc_outputs_class_unselected = self.lang_encoder.compute_similarity(
                output_memory_
            )
            enc_outputs_class_unselected[output_proposals.sum(-1).isinf()] = float(
                "-inf"
            )
            ...
            outputs_class, outputs_mask = self.forward_prediction_heads(
                tgt_undetach.transpose(0, 1), mask_features, do_seg
            )
            ...
            if self.initialize_box_type != "no" and do_seg:
                ...
                if self.initialize_box_type == "bitmask":  # slower, but more accurate
                    refpoint_embed = (
                        BitMasks(flaten_mask > 0).get_bounding_boxes().tensor.cuda()
                    )
        ...
        if self.initial_pred:
            outputs_class, outputs_mask = self.forward_prediction_heads(
                tgt.transpose(0, 1), mask_features, self.training and do_seg
            )
        ...
        if mask_dict is not None:
            ...
            if predictions_mask is None:
                predictions_class[-1] = (
                    predictions_class[-1] + 0.0 * self.lang_mapper[0, 0]
                )
...
@register_decoder
def get_maskdino_transformer_decoder(
