...
def vl_multilabel_contrastive_loss(image_feat, text_feat, temperature=1):
    ...
    labels_per_img *= rearrange(
        pos_labels_batch_img, "b l1 l2 -> b l1 1 l2 1"
    ) * repeat(
        torch.eye(batch, dtype=image_x.dtype, device=image_x.device),
        "b1 b2 -> b1 1 b2 1 1",
    )
    # [BxL1, WxBxL2]
    ...
    labels_per_text *= rearrange(
        pos_labels_batch_text, "b l2 l1 -> b l2 1 l1 1"
    ) * repeat(
        torch.eye(batch, dtype=text_x.dtype, device=image_x.device),
        "b2 b1 -> b2 1 b1 1 1",
    )
    # [BxL2, WxBxL1]
...
def ql_multi_contrastive_loss(image_feat, text_feat, text_hash, temperature=1):
    ...
    text_feat_unique = torch.stack(
        [text_feat[text_hash_all.index(txt)] for txt in text_hash_all_unique]
    )
...
def image_text_contrastive_loss_queue(
    image_feat_inp, text_feat_inp, lang_enc, training
):
    # add the following 4 lines
